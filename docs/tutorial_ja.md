...
各計画には以下が含まれます：
- **ゴール**：元のタスクの説明
- **ステップ**：最大3〜8の順次ステップ（適応的）、各ステップには：
  - タイトルと詳細な指示
  - 成功基準（検証可能な条件）
  - エージェントステップの予算（各8〜15ステップ、適応的）
- **前提条件**：プランナーが環境について想定した内容

### 複雑度適応プランニング（v0.5.0）

プランナーはゴールの複雑度（Low / Medium / High）を自動推定し、動作を調整します：

| 複雑度 | 最大ステップ数 | エージェントステップ予算 | 再計画回数 |
|--------|---------------|------------------------|-----------|
| Low | 3 | 各8ステップ | 0回 |
| Medium | 5 | 各12ステップ | 1回 |
| High | 8 | 各15ステップ | 2回 |

「typo を修正」のような単純なゴールにはステップ数と予算を抑え、「データベーススキーマをリファクタリングしてデータを移行」のような複雑なゴールには十分な余裕を与えます。
...
3. ステップが失敗した場合 → 複雑度に応じた回数まで再計画、その後フォールバック
...
3. ファイル書き込み10回ごと、および各計画ステップ後に**スナップショット**を取得
...
小さな LLM では想定される動作です。補償エンジンがエラークラス別の戦略で自動的に対処します（v0.5.0）：

| エラークラス | 対処戦略 |
|------------|---------|
| `malformed_json` | JSON 修復（LLM リトライ不要） |
| `wrong_tool_name` | ファジーマッチで正しいツール名を提案 |
| `missing_args` | パラメータスキーマを補正プロンプトに注入 |
| `empty_response` | 即座にモデルエスカレーション |
| `prose_wrapped` | 散文から JSON を抽出（パーサーが対処） |

クラス別リカバリーが失敗した場合、標準のリトライシーケンスが適用されます：
...
Open Harness には [Textual](https://textual.textualize.io/) を使用したリッチなターミナルUI（TUI）が含まれています。以下のコマンドで起動します：

```bash
harness --tui
```

> **前提条件**: TUI エクストラをインストール: `uv pip install -e ".[tui]"`

### レイアウト

TUI は2ペイン構成です：

- **メイン出力**（左）— エージェントの出力、ツール呼び出し、結果のストリーミング表示
- **サイドバー**（右）— 履歴、計画、キュー、エージェント、タスク、統計の折りたたみ可能セクション
- **サブエージェントパネル** — 外部エージェント（Claude Code, Codex, Gemini）がアクティブな時に自動表示
- **プログレスバー** — ゴール実行中に計画ステップの進捗を表示（例：Step 2/5）
- **入力エリア**（下部）— Shift+Enter で改行可能なマルチライン テキストエリア

ペイン間の**ドラッグハンドル**をクリック＆ドラッグして幅を調整できます。

### キーボードショートカット

| キー | アクション |
|------|-----------|
| `Escape` | 実行中のエージェントをキャンセル |
| `Enter` | 入力を送信 |
| `Shift+Enter` | 入力に改行を挿入 |
| `↑` / `↓` | 入力履歴のナビゲーション（カーソルが先頭行/最終行の場合） |
| `Tab` | モード切替: plan / goal（入力フォーカス外の場合） |
| `Ctrl+E` | ツール出力のコンパクト（200文字）/ 展開（2000文字）切替 |
| `Ctrl+D` | ダーク / ライトテーマ切替 |
| `Ctrl+S` | サイドバーの表示/非表示 |
| `Ctrl+A` | サブエージェントパネルの表示/非表示 |
| `Ctrl+L` | 出力ログをクリア |
| `Ctrl+Q` | 終了 |
| `F1` | ヘルプを表示 |
...
**エージェントのキャンセル** — `Escape` キーで実行中のエージェントをキャンセルできます。現在のツール呼び出しが完了後に停止します。CLIモードでは、plan/goal 実行中に `Ctrl+C` で同じ機能を提供します。

**入力履歴** — `↑` / `↓` 矢印キーで過去の入力を遡って参照できます。plan モードと goal モードの両方の入力が含まれます。サイドバーの履歴セクションのエントリをクリックすると、その出力位置までスクロールします。

**プランステップ進捗バー** — ゴール実行中、出力エリアの上部にプログレスバーが表示され、現在のステップと全ステップ数を示します（例：`Step 3/5`）。サイドバーの計画セクションでは、完了済みステップが緑、実行中が黄色、未実行がグレーで表示されます。

**コンパクト / 展開ツール出力** — デフォルトでは、ツール結果は200文字に切り詰められます。`Ctrl+E` でコンパクト（200文字）と展開（2000文字）モードを切り替えられます。

**リアルタイム統計** — 統計サイドバーセクションはエージェント実行中に毎秒更新され、経過時間、ツール呼び出し回数、補償回数を表示します。

**トースト通知** — バックグラウンドタスクが完了すると、画面上部にトースト通知が表示され、成功または失敗を示します。

**ダーク / ライトテーマ** — `Ctrl+D` でダークモードとライトモードを切り替えられます。

**マルチライン入力** — 入力エリアはフルテキストエディタです。`Enter` で送信、`Shift+Enter` で改行を追加します。複数行のコードを貼り付けたり、複雑なプロンプトを書く際に便利です。

### TUI でのスラッシュコマンド

すべての REPL コマンドが TUI モードで使用可能です: `/goal`、`/submit`、`/tasks`、`/result`、`/model`、`/tier`、`/policy`、`/tools`、`/memory`、`/project`、`/clear`、`/quit`
